                ...
                <div className='flex-1 overflow-hidden px-4 py-2'>
                    {
                        showAllUsers ? <div className='space-y-4 h-full'>
                            <div className='relative'>
                                ...
                                <input type="text" placeholder='Search User...' className='w-full pl-10 py-3 bg-gray-800 border border-gray-700 text-white placeholder-gray-400 outline-0 rounded-lg'
                                    value={searchQuery}
                            ...
                            <div className='space-y-2 overflow-y-auto h-full pb-4'>
                                {
                                    users?.filter((u) => u._id !== loggedInUser?._id && u.name.toLowerCase().includes(searchQuery.toLowerCase())).map((u) => (
                                        <button key={u._id} className='w-full text-left rounded-lg p-4 border border-gray-700 hover:border-gray-600 hover:bg-gray-800 transition-colors ' onClick={()=>createChat(u)}>
                                            <div className='flex items-center gap-3'>
                                                <div className='relative'>
                        ...
                        </div> : (chats && chats.length > 0 ? <div className='space-y-2 overflow-y-auto h-full pb-4'>
                            {
                                chats.map((chat) => {
                                    ...
                                    }} className={`w-full text-left p-4 rounded-lg transition-color ${isSelected ? "bg-blue-600 border border-blue-500" : "border border-gray-700 hover:border-gray-600"}`} >
                                        <div className='flex items-center gap-3 '>
                                            <div className='relative'>
                                                ...
                                                </div>
     {
        onlineUsers.includes(chat.user._id) && (
            <span className='absolute -top-0.5 -right-0.5 w-3 h-3 rounded-full bg-green-500 border-2 border-gray-900 '/>
                ...
                {/* Footer  */}
                <div className='p-4 border-t border-gray-700 space-y-2'><Link href={"/profile"} className='flex items-center gap-3 px-4 py-3 rounded-lg hover:bg-gray-800 transition-colors'>

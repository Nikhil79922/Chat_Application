...
const ProfilePage = () => {
    ...
    const submitHandler = async (e: any) => {
        ...
        try {
            const { data } = await axios.post(`${user_service}/api/v1/updateName`, { updatedName:name },
                {
                    headers: {
        ...
        <div className='min-h-screen bg-gray-900 p-4 '>
            <div className="max-w-2xl mx-auto pt-8 ">
                <div className="flex items-center gap-4 mb-8">
                    <button className='p-3 bg-gray-800 hover:bg-gray-700 rounded-lg border border-gray-700 ' onClick={() => router.push("/chat")}><ArrowLeft className='w-5 h-5 text-gray-300' /> </button>
                    <div> <h1 className='text-3xl font-bold text-white'>Profile settings</h1>
                            ...
                            <div>
                                <label className='block text-sm font-semibold text-gray-300 mb-3 '>Display name</label>
                                {
                                    isEdit ? <form onSubmit={submitHandler} className='space-y-4  '>
                                        ...
                                        <div className='flex gap-3 '><button type='submit' className='flex items-center gap-2 px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white font-semibold rounded-lg cursor-pointer'><Save className='w-4 h-4'/>Save Changes
                                            </button>
                                            <button type='button' onClick={editHandler} className='flex items-center gap-2 px-6 py-3 bg-gray-600 hover:bg-gray-700 text-white font-semibold rounded-lg cursor-pointer'><Save className='w-4 h-4'/>Cancel
                                            </button>
                                        ...
                                        <button onClick={editHandler} className='flex items-center gap-2 px-6 py-3 bg-gray-600 hover:bg-gray-700 text-white font-semibold rounded-lg cursor-pointer'>Edit
                                        </button>
